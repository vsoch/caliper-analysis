...
with tf.Session() as sess:
    ...
    for i in range(1, num_steps + 1):
        ...
        _, _, gl, dl = sess.run(
            [train_gen, train_disc, gen_loss, disc_loss], feed_dict=feed_dict
        )
        if i % 100 == 0 or i == 1:
            print("Step %i: Generator Loss: %f, Discriminator Loss: %f" % (i, gl, dl))

    # Generate image from (seeded) noise, using the generator network.
    z = np.random.uniform(-1.0, 1.0, size=[4, noise_dim])
    g = sess.run(gen_sample, feed_dict={noise_input: z})
    img = np.reshape(np.repeat(g[1][:, :, np.newaxis], 3, axis=2), newshape=(28, 28, 3))

    # Modified to just print single result
    print([list(x) for x in img])
